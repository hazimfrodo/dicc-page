...
export default function Services() {
  return (
    <section id="services" className="relative py-20 md:py-28 bg-[#0d1927] overflow-hidden">
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6">
        <ScrollParallax speed={0.1}>
          <div className="text-center mb-16">
            ...
            <h2 className="text-3xl md:text-4xl lg:text-5xl font-bold text-white max-w-3xl mx-auto leading-tight">
              Comprehensive Research Computing Solutions
            </h2>
          </div>
        </ScrollParallax>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {services.map((service, i) => (
            <ScrollParallax key={service.title} speed={0.06 + (i % 3) * 0.03}>
              <CursorParallax speed={6}>
                <a
                  href={service.href}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="group p-7 rounded-2xl bg-[#192f59]/50 backdrop-blur-sm border border-[#192f59] hover:border-[#C8A951]/50 transition-all duration-500 hover:-translate-y-1 hover:shadow-xl hover:shadow-[#C8A951]/10 block h-full"
                >
                  <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-[#C8A951] to-[#A8893D] flex items-center justify-center text-[#061a3a] mb-5 group-hover:scale-110 transition-transform duration-300">
                    {service.icon}
                  </div>
